...
int main()
{
    ...
    int N,M,c,StarEdge,EndEdge,source,sink;
    ...
    for(int EdgeNumber=0; EdgeNumber<M; EdgeNumber++)
    {
        ...
        g.addEdge(StarEdge,EndEdge,c);
        ResidualGraph.addEdge(StarEdge,EndEdge,c);
    ...
    ofstream outputfile;
    outputfile.open("flow_output.txt");
    int MaximumFlow=ResidualGraph.FordFulkerson(source,sink);
    cout<<MaximumFlow<<endl;
    outputfile<<MaximumFlow<<"\n";

    for(int u=0; u<N; u++)
    {
        for(int v=1; v<g.adjList[u].getLength(); v++)
        {
            int MainGraphEdgeCapacity=g.adjList[u].List[v].GetEdgeWeight();
            int ResidualGraphEdgeCapacity=ResidualGraph.adjList[u].List[v].GetEdgeWeight();
            int EachEdgeFlow=MainGraphEdgeCapacity-ResidualGraphEdgeCapacity;

            int itemUsingPosition=g.adjList[u].getItem(v);
            cout<<u<<" "<<itemUsingPosition<<" "<<EachEdgeFlow<<"/"<<MainGraphEdgeCapacity<<endl;
            outputfile<<u<<" "<<itemUsingPosition<<" "<<EachEdgeFlow<<"/"<<MainGraphEdgeCapacity<<"\n";
